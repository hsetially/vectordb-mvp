...
def test_insert_builds_graph(hnsw_index):
    ...
    assert total_connections > 0, "Graph should have at least one connection"
...
def test_graph_connectivity_limits(hnsw_index):
    ...
    for node_id, neighbors in hnsw_index.graphs[0].items():
        assert len(neighbors) <= limit_0, \
            f"Node {node_id} on layer 0 has {len(neighbors)} connections, exceeds limit {limit_0}"
    
    if hnsw_index.max_layer > 0:
        limit_high = hnsw_index.M_max
        for layer_num in range(1, hnsw_index.max_layer + 1):
            for node_id, neighbors in hnsw_index.graphs[layer_num].items():
                assert len(neighbors) <= limit_high, \
                    f"Node {node_id} on layer {layer_num} has {len(neighbors)} connections, exceeds limit {limit_high}"
...
def test_layer_selection_distribution(hnsw_index):
    ...
    if 0 in layer_counts and 1 in layer_counts:
        assert layer_counts[1] < layer_counts[0], \
            f"Layer 1 count {layer_counts[1]} should be less than layer 0 count {layer_counts[0]}"
    
    if 1 in layer_counts and 2 in layer_counts:
        assert layer_counts[2] < layer_counts[1], \
            f"Layer 2 count {layer_counts[2]} should be less than layer 1 count {layer_counts[1]}"
...
def test_stress_insert_performance():
    ...
    print(f"\nInserted {num_vectors} vectors in {duration:.2f} seconds ({num_vectors/duration:.0f} vec/sec)")
    
    assert duration < 2.0, f"Insertion took {duration:.2f}s, should be < 2.0s"
    assert len(index.vectors) == num_vectors, f"Expected {num_vectors} vectors, got {len(index.vectors)}"
    assert len(index.graphs[0]) == num_vectors, f"Expected {num_vectors} nodes on layer 0, got {len(index.graphs[0])}"
